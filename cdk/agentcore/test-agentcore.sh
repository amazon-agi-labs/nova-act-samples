...
npx cdk destroy --app "npx ts-node agentcore-app.ts" --force || true

echo "Deploying fresh stack (CDK will build container)..."
npx cdk deploy --app "npx ts-node agentcore-app.ts" --require-approval never
...
npx cdk destroy --app "npx ts-node agentcore-app.ts" --force
